...
for i=1:numel(coloc_analysis_Rep1)
    coloc_analysis_Rep1{i}(coloc_analysis_Rep1{i}(:)==Inf)=nan;
end
...
for i=1:numel(coloc_analysis_Rep2)
    coloc_analysis_Rep2{i}(coloc_analysis_Rep2{i}(:)==Inf)=nan;
end
...
for i=1:numel(STEDCorr)
    STEDCorr{i}(STEDCorr{i}(:)==Inf)=nan;
end
STEDCorr_UID=STEDCorr(:,1);

% Read in spine type correction file
tmp=load('Z:\user\mhelm1\Nanomap_Analysis\Data\total\SpineTypeCorrection.mat');
...
type_corr_num(isinf(type_corr_num))=NaN;
...
for i=1:numel(folders)
    ...
    results{i+1,copynum_from_Homer_SEM_indx}=results{i+1,copynum_from_Homer_indx}*sqrt(max((results{i+1,4}/results{i+1,3}),0)^2+max((results{i+1,col_ratio_SEM_mean_indx}/results{i+1,col_ratio_mean_indx}),0)^2+max((results{i+1,STEDCorr_SEM_indx}/results{i+1,STEDCorr_indx}),0)^2); %synaptic copy number SEM
    
    %     Manually enter the two copy numbers that I get from Schanzenbächer et al. Neuron, 2016
    if strcmp('Igf1r',UID) %IGF1R
        results{i+1,copynum_from_Homer_indx}=53.9278539703109;
        results{i+1,mol_weight_indx}=155396;
        %extrapolate whole-cell number from synaptic number. Since they
        %dont report an error for their value, I only use the SEM from
        %colocalization and STED correlation here
        results{i+1,3} = (results{i+1, copynum_from_Homer_indx} * spinenumber) / (results{i+1,col_ratio_mean_indx} * results{i+1,STEDCorr_indx} * 0.01);
        results{i+1,4} = results{i+1,3} * sqrt(max((results{i+1,col_ratio_SEM_mean_indx}/results{i+1,col_ratio_mean_indx}),0)^2 + max((results{i+1,STEDCorr_SEM_indx}/results{i+1,STEDCorr_indx}),0)^2);
        
    elseif strcmp('Arc',UID) %Arc
        results{i+1,copynum_from_Homer_indx}=392.883758408924;
        results{i+1,mol_weight_indx}=45353;
        %extrapolate whole-cell number from synaptic number. Since they
        %dont report an error for their value, I only use the SEM from
        %colocalization and STED correlation here
        results{i+1,3} = (results{i+1, copynum_from_Homer_indx} * spinenumber) / (results{i+1,col_ratio_mean_indx} * results{i+1,STEDCorr_indx} * 0.01);
    ...
    if sum(strcmp(UID,{'Shank1','Shank2','Shank3','Myo5a','Cacna1a','Sptbn2','Grin1','Grin2b','Gria1','Gria2','Gria3','Grm2','Grm5','Ntrk2','Map2'})) %The following proteins I would also list here, but where not detected by Benni: Grin2a, Grik1, Grm1, Drd1, Drd2, Igf1r, LNGFR, nAChR beta 2
        ...
        %also extrapolate correct whole cell copy numbers from the
        %corrected synaptic copy number
        results{i+1,3} = (results{i+1, copynum_from_Homer_indx} * spinenumber) / (results{i+1,col_ratio_mean_indx} * results{i+1,STEDCorr_indx} * 0.01);
        results{i+1,4} = results{i+1,3} * sqrt(max((results{i+1,copynum_from_Homer_SEM_indx}/results{i+1, copynum_from_Homer_indx}),0)^2 + max((results{i+1,col_ratio_SEM_mean_indx}/results{i+1,col_ratio_mean_indx}),0)^2 + max((results{i+1,STEDCorr_SEM_indx}/results{i+1,STEDCorr_indx}),0)^2);
    end
...
cd(the_folder);
xlswrite(['SynapticCopynumbers_Extended_results_SE' num2str(SE_erode) '.xlsx'],results);
xlswrite(['THE COPY NUMBER FILE_SE' num2str(SE_erode) '.xlsx'],synaptosome_data);
