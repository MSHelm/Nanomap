...
thefolder='C:\data_2012\spines\spine_3Ds\mushroom';
...
matrix=[];matrix_major=[]; matrix_minor=[]; matrix_mems=[];
...
% col12= number mitochondria objects
...
if stat==1
    ...
    siza=size(matrix);
    matrix(siza(1)+1,1)=sum(val_matrix(:,1));
    matrix(siza(1)+1,2)=sum(val_matrix(:,6));
    matrix(siza(1)+1,3)=sum(val_matrix(:,13).*val_matrix(:,16));
    matrix(siza(1)+1,4)=sum(val_matrix(:,9).*val_matrix(:,12));
    matrix(siza(1)+1,5)=sum(val_matrix(:,5));
    matrix(siza(1)+1,6)=sum(val_matrix(:,16));    
...
figure;
subplot(2,2,1);hist(matrix(:,1)); subplot(2,2,2); hist(matrix(:,2)); subplot(2,2,3); hist(matrix(:,3)); subplot(2,2,4); hist(matrix(:,4));title(thefolder);
