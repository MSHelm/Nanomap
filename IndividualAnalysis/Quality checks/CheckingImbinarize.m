...
dio_bw=imopen(dio_bw,strel('disk',2));
        ...
        ccc=find(leftin~=1); homer(ccc)=0;
         ...
         homer_bw=imbinarize(mat2gray(homer),factor*otsuthresh(mat2gray(homer(:))));
%        homer_bw=imbinarize(mat2gray(homer),'adaptive','Sensitivity',0.2);
        subplot(4,8,i); imagesc(homer); subplot(4,8,i+8); imagesc(homer_bw); subplot(4,8,i+16); imagesc(dio); subplot(4,8,i+24); imagesc(dio_bw);
        i=i+1;
        if i==8
            break
        end
