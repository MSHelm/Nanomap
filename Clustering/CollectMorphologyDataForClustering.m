...
for i=1:numel(folders)
    ...
    %         only one entry per spot, this doesnt do anything. 
    %         This messes up the SpotFile entry, but this doesnt matter, as I save it
    %         in a separate file.
    for j=1:numel(fields)
        if strcmp(fields{j},'SpotFile')
            temp_mush(:,j)=padcat(results_total.(fields{j}){class==1});
            temp_flat(:,k)=(padcat(results_total.(fields{j}){class==2});
            temp_other(:,j)=padcat(results_total.(fields{j}){class==3});
            continue
        end
