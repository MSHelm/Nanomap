function align_images_imwarp_parallelized_nofilt_fullimg(cd_path)
...
for treatment=3:numel(folders)
    ...
    if exist('bottomx.txt')==2 && exist('Mush_dio_average_nofilt_fullimg.txt')==0
        ...
        %% Create variables
        Mush_sted_average=zeros(501);
        Mush_dio_average=zeros(501);
        Mush_homer_average=zeros(501);
        Flat_sted_average=zeros(501);
        Flat_dio_average=zeros(501);
        Flat_homer_average=zeros(501);
        Other_sted_average=zeros(501);
        Other_dio_average=zeros(501);
        Other_homer_average=zeros(501);
        Mush_sted_resized_average=zeros(501);
        Mush_dio_resized_average=zeros(501);
        Mush_homer_resized_average=zeros(501);
        Flat_sted_resized_average=zeros(501);
        Flat_dio_resized_average=zeros(501);
        Flat_homer_resized_average=zeros(501);
        Other_sted_resized_average=zeros(501);
        Other_dio_resized_average=zeros(501);
        Other_homer_resized_average=zeros(501);
        Mush_sted_noback_average=zeros(501);
        Mush_dio_noback_average=zeros(501);
        Mush_homer_noback_average=zeros(501);
        Flat_sted_noback_average=zeros(501);
        Flat_dio_noback_average=zeros(501);
        Flat_homer_noback_average=zeros(501);
        Other_sted_noback_average=zeros(501);
        Other_dio_noback_average=zeros(501);
        Other_homer_noback_average=zeros(501);
        Mush_sted_norm2shaft_average=zeros(501);
        Mush_dio_norm2shaft_average=zeros(501);
        Mush_homer_norm2shaft_average=zeros(501);
        Flat_sted_norm2shaft_average=zeros(501);
        Flat_dio_norm2shaft_average=zeros(501);
        Flat_homer_norm2shaft_average=zeros(501);
        Other_sted_norm2shaft_average=zeros(501);
        Other_dio_norm2shaft_average=zeros(501);
        Other_homer_norm2shaft_average=zeros(501);
        Mush_sted_noback_norm2shaft_average=zeros(501);
        Mush_dio_noback_norm2shaft_average=zeros(501);
        Mush_homer_noback_norm2shaft_average=zeros(501);
        Flat_sted_noback_norm2shaft_average=zeros(501);
        Flat_dio_noback_norm2shaft_average=zeros(501);
        Flat_homer_noback_norm2shaft_average=zeros(501);
        Other_sted_noback_norm2shaft_average=zeros(501);
        Other_dio_noback_norm2shaft_average=zeros(501);
        Other_homer_noback_norm2shaft_average=zeros(501);
        ...
        parfor i=1:numel(mess)
            ...
            if exist(['sted_aligned_nofilt_fullimg_' num2str(i) '.txt'],'file')==2
%                 %% if spot was already analyzed read in the files and add to the averages
                sted_aligned=dlmread(strcat('sted_aligned_nofilt_fullimg_',num2str(i),'.txt'));
                dio_aligned=dlmread(strcat('dio_aligned_nofilt_fullimg_',num2str(i),'.txt'));
                homer_aligned=dlmread(strcat('homer_aligned_nofilt_fullimg_',num2str(i),'.txt'));
                %                 sted_aligned_resized=dlmread(strcat('sted_aligned_resized_nofilt_fullimg_',num2str(i),'.txt'));
                %                 dio_aligned_resized=dlmread(strcat('dio_aligned_resized_nofilt_fullimg_',num2str(i),'.txt'));
                %                 homer_aligned_resized=dlmread(strcat('homer_aligned_resized_nofilt_fullimg_',num2str(i),'.txt'));
                sted_aligned_noback=dlmread(strcat('sted_aligned_noback_nofilt_fullimg_',num2str(i),'.txt'));
                dio_aligned_noback=dlmread(strcat('dio_aligned_noback_nofilt_fullimg_',num2str(i),'.txt'));
                homer_aligned_noback=dlmread(strcat('homer_aligned_noback_nofilt_fullimg_',num2str(i),'.txt'));
                sted_aligned_norm2shaft=dlmread(strcat('sted_aligned_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'));
                dio_aligned_norm2shaft=dlmread(strcat('dio_aligned_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'));
                homer_aligned_norm2shaft=dlmread(strcat('homer_aligned_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'));
                sted_aligned_noback_norm2shaft=dlmread(strcat('sted_aligned_noback_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'));
                dio_aligned_noback_norm2shaft=dlmread(strcat('dio_aligned_noback_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'));
                homer_aligned_noback_norm2shaft=dlmread(strcat('homer_aligned_noback_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'));
                ...
                if classification(i)<4
                    try
                        ...
                        filt_db=di_b;
                        di_bw=imbinarize(mat2gray(di_b));
%                         filt_db=di_b;ccc=find(di_b>0);
%                         pp=hist(di_b(ccc),[0.5:1:50]); ppm=max(pp); cccp=find(pp==ppm,1);
%                         ccc=find(di_b<cccp+std2(di_b)); di_b(ccc)=0;
%                         ccc=find(di_b>0); di_bw=di_b; di_bw(ccc)=1; di_bw=imerode(di_bw,strel('disk',1));
%                         
                        %Old: filter STED signal to remove noise and set everything outside DiO mask to 0
                        %{
    ...
    ccc=find(sted3<mean2(sted3)+4); sted3(ccc)=0;
                        %}
                        ccc=find(di_bw==0); homer(ccc)=0; sted(ccc)=0; filt_db(ccc)=0;
                        
                        
                        %to keep syntax later the same, define sted3 as the
                        %original sted image here again.
                        sted3=sted;
...
rot_shifted_homer_noback=imrotate(shifted_homer_noback,(angle));
                    ...
                    %old: take only central 150 pixels
                    %{
...
%                         homer_aligned_resized=rot_shifted_scaled_homer(startpoint2:startpoint2+150,startpoint2:startpoint2+150);
                    ...
                    try
                        dlmwrite(strcat('sted_aligned_nofilt_fullimg_',num2str(i),'.txt'),sted_aligned);
                        dlmwrite(strcat('dio_aligned_nofilt_fullimg_',num2str(i),'.txt'),dio_aligned);
                        dlmwrite(strcat('homer_aligned_nofilt_fullimg_',num2str(i),'.txt'),homer_aligned);
                        %dlmwrite(strcat('sted_aligned_resized_nofilt_fullimg_',num2str(i),'.txt'),sted_aligned_resized);
                        %dlmwrite(strcat('dio_aligned_resized_nofilt_fullimg_',num2str(i),'.txt'),dio_aligned_resized);
                        %dlmwrite(strcat('homer_aligned_resized_nofilt_fullimg_',num2str(i),'.txt'),homer_aligned_resized);
                        dlmwrite(strcat('sted_aligned_noback_nofilt_fullimg_',num2str(i),'.txt'),sted_aligned_noback);
                        dlmwrite(strcat('dio_aligned_noback_nofilt_fullimg_',num2str(i),'.txt'),dio_aligned_noback);
                        dlmwrite(strcat('homer_aligned_noback_nofilt_fullimg_',num2str(i),'.txt'),homer_aligned_noback);
                        dlmwrite(strcat('sted_aligned_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'),sted_aligned_norm2shaft);
                        dlmwrite(strcat('dio_aligned_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'),dio_aligned_norm2shaft);
                        dlmwrite(strcat('homer_aligned_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'),homer_aligned_norm2shaft);
                        dlmwrite(strcat('sted_aligned_noback_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'),sted_aligned_noback_norm2shaft);
                        dlmwrite(strcat('dio_aligned_noback_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'),dio_aligned_noback_norm2shaft);
                        dlmwrite(strcat('homer_aligned_noback_norm2shaft_nofilt_fullimg_',num2str(i),'.txt'),homer_aligned_noback_norm2shaft);
                        
                        %save the rectangle!
                        
                        dlmwrite(strcat('rectangle_shaft_sted_nofilt_fullimg_',num2str(i),'.txt'),rectangle_shaft_sted);
                        dlmwrite(strcat('rectangle_shaft_dio_nofilt_fullimg_',num2str(i),'.txt'),rectangle_shaft_dio);
                        dlmwrite(strcat('rectangle_shaft_homer_nofilt_fullimg_',num2str(i),'.txt'),rectangle_shaft_homer);
        ...
        %% Write files
        dlmwrite('Mush_sted_average_nofilt_fullimg.txt',(Mush_sted_average./(sum(classification(:)==1))));
        dlmwrite('Mush_dio_average_nofilt_fullimg.txt',(Mush_dio_average./(sum(classification(:)==1))));
        dlmwrite('Mush_homer_average_nofilt_fullimg.txt',(Mush_homer_average./(sum(classification(:)==1))));
        
        %dlmwrite('Mush_sted_resized_average_nofilt_fullimg.txt',(Mush_sted_resized_average./(sum(classification(:)==1))));
        %dlmwrite('Mush_dio_resized_average_nofilt_fullimg.txt',(Mush_dio_resized_average./(sum(classification(:)==1))));
        %dlmwrite('Mush_homer_resized_average_nofilt_fullimg.txt',(Mush_homer_resized_average./(sum(classification(:)==1))));
        
        dlmwrite('Mush_sted_noback_average_nofilt_fullimg.txt',(Mush_sted_noback_average./(sum(classification(:)==1))));
        dlmwrite('Mush_dio_noback_average_nofilt_fullimg.txt',(Mush_dio_noback_average./(sum(classification(:)==1))));
        dlmwrite('Mush_homer_noback_average_nofilt_fullimg.txt',(Mush_homer_noback_average./(sum(classification(:)==1))));
        
        dlmwrite('Mush_sted_norm2shaft_average_nofilt_fullimg.txt',(Mush_sted_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Mush_dio_norm2shaft_average_nofilt_fullimg.txt',(Mush_dio_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Mush_homer_norm2shaft_average_nofilt_fullimg.txt',(Mush_homer_norm2shaft_average./(sum(classification(:)==1))));
        
        dlmwrite('Mush_sted_noback_norm2shaft_average_nofilt_fullimg.txt',(Mush_sted_noback_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Mush_dio_noback_norm2shaft_average_nofilt_fullimg.txt',(Mush_dio_noback_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Mush_homer_noback_norm2shaft_average_nofilt_fullimg.txt',(Mush_homer_noback_norm2shaft_average./(sum(classification(:)==1))));
        
        dlmwrite('Flat_sted_average_nofilt_fullimg.txt',(Flat_sted_average./(sum(classification(:)==2))));
        dlmwrite('Flat_dio_average_nofilt_fullimg.txt',(Flat_dio_average./(sum(classification(:)==2))));
        dlmwrite('Flat_homer_average_nofilt_fullimg.txt',(Flat_homer_average./(sum(classification(:)==2))));
        
        dlmwrite('Flat_sted_resized_average_nofilt_fullimg.txt',(Flat_sted_resized_average./(sum(classification(:)==2))));
        dlmwrite('Flat_dio_resized_average_nofilt_fullimg.txt',(Flat_dio_resized_average./(sum(classification(:)==2))));
        dlmwrite('Flat_homer_resized_average_nofilt_fullimg.txt',(Flat_homer_resized_average./(sum(classification(:)==2))));
        
        dlmwrite('Flat_sted_noback_average_nofilt_fullimg.txt',(Flat_sted_noback_average./(sum(classification(:)==2))));
        dlmwrite('Flat_dio_noback_average_nofilt_fullimg.txt',(Flat_dio_noback_average./(sum(classification(:)==2))));
        dlmwrite('Flat_homer_noback_average_nofilt_fullimg.txt',(Flat_homer_noback_average./(sum(classification(:)==2))));
        
        dlmwrite('Flat_sted_norm2shaft_average_nofilt_fullimg.txt',(Flat_sted_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Flat_dio_norm2shaft_average_nofilt_fullimg.txt',(Flat_dio_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Flat_homer_norm2shaft_average_nofilt_fullimg.txt',(Flat_homer_norm2shaft_average./(sum(classification(:)==1))));
        
        dlmwrite('Flat_sted_noback_norm2shaft_average_nofilt_fullimg.txt',(Flat_sted_noback_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Flat_dio_noback_norm2shaft_average_nofilt_fullimg.txt',(Flat_dio_noback_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Flat_homer_noback_norm2shaft_average_nofilt_fullimg.txt',(Flat_homer_noback_norm2shaft_average./(sum(classification(:)==1))));
        
        dlmwrite('Other_sted_average_nofilt_fullimg.txt',(Other_sted_average./(sum(classification(:)==3))));
        dlmwrite('Other_dio_average_nofilt_fullimg.txt',(Other_dio_average./(sum(classification(:)==3))));
        dlmwrite('Other_homer_average_nofilt_fullimg.txt',(Other_homer_average./(sum(classification(:)==3))));
        
        dlmwrite('Other_sted_resized_average_nofilt_fullimg.txt',(Other_sted_resized_average./(sum(classification(:)==3))));
        dlmwrite('Other_dio_resized_average_nofilt_fullimg.txt',(Other_dio_resized_average./(sum(classification(:)==3))));
        dlmwrite('Other_homer_resized_average_nofilt_fullimg.txt',(Other_homer_resized_average./(sum(classification(:)==3))));
        
        dlmwrite('Other_sted_noback_average_nofilt_fullimg.txt',(Other_sted_noback_average./(sum(classification(:)==3))));
        dlmwrite('Other_dio_noback_average_nofilt_fullimg.txt',(Other_dio_noback_average./(sum(classification(:)==3))));
        dlmwrite('Other_homer_noback_average_nofilt_fullimg.txt',(Other_homer_noback_average./(sum(classification(:)==3))));
        
        dlmwrite('Other_sted_norm2shaft_average_nofilt_fullimg.txt',(Other_sted_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Other_dio_norm2shaft_average_nofilt_fullimg.txt',(Other_dio_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Other_homer_norm2shaft_average_nofilt_fullimg.txt',(Other_homer_norm2shaft_average./(sum(classification(:)==1))));
        
        dlmwrite('Other_sted_noback_norm2shaft_average_nofilt_fullimg.txt',(Other_sted_noback_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Other_dio_noback_norm2shaft_average_nofilt_fullimg.txt',(Other_dio_noback_norm2shaft_average./(sum(classification(:)==1))));
        dlmwrite('Other_homer_noback_norm2shaft_average_nofilt_fullimg.txt',(Other_homer_noback_norm2shaft_average./(sum(classification(:)==1))));
        
        dlmwrite('vertical_shifts_nofilt_fullimg.txt',vertical_shifts);
        dlmwrite('horizontal_shifts_nofilt_fullimg.txt',horizontal_shifts);
        dlmwrite('rotation_angles_nofilt_fullimg.txt',rotation_angles);
    else
