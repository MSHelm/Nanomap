function align_images_imwarp_parallelized(cd_path)
...
%             
%             if exist(['sted_aligned_' num2str(i) '.txt'],'file')==2
...
%                     
                ...
                try
                    ...
                    %create tform
                    Rdefault = imref2d(size(shifted_sted));
...
%                     rot_shifted_scaled_homer=imwarp(imresize(shifted_homer,(scale_by/(abs(topy(i)-bottomy(i))))),tform);
                ...
                try
                    % sted_aligned=rot_shifted_sted;
                    % dio_aligned=rot_shifted_dio;
                    % homer_aligned=rot_shifted_homer;
                    % sted_aligned_resized=rot_shifted_scaled_sted;
                    % dio_aligned_resized=rot_shifted_scaled_dio;
                    % homer_aligned_resized=rot_shifted_scaled_homer;
                    % sted_aligned_noback=rot_shifted_sted_noback;
                    % dio_aligned_noback=rot_shifted_dio_noback;
                    % homer_aligned_noback=rot_shifted_homer_noback;
                    % sted_aligned_norm2shaft=rot_shifted_sted_normalized2shaft;
                    % dio_aligned_norm2shaft=rot_shifted_dio_normalized2shaft;
                    % homer_aligned_norm2shaft=rot_shifted_homer_normalized2shaft;
                    % sted_aligned_noback_norm2shaft=rot_shifted_sted_noback_normalized2shaft;
                    % dio_aligned_noback_norm2shaft=rot_shifted_dio_noback_normalized2shaft;
                    % homer_aligned_noback_norm2shaft=rot_shifted_homer_noback_normalized2shaft;
...
%                     dlmwrite(strcat('homer_aligned_resized_',num2str(i),'.txt'),homer_aligned_resized);
                    ...
                    dlmwrite(strcat('rectangle_shaft_sted_',num2str(i),'.txt'),rectangle_shaft_sted);
                    dlmwrite(strcat('rectangle_shaft_dio_',num2str(i),'.txt'),rectangle_shaft_dio);
                    dlmwrite(strcat('rectangle_shaft_homer_',num2str(i),'.txt'),rectangle_shaft_homer);
                catch
...
%             dlmwrite('Other_homer_resized_average.txt',(Other_homer_resized_average./(sum(classification(:)==3))));
            ...
            disp('Error writing the averages')
                        continue
        end
